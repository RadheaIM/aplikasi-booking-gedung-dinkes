<x-guest-layout>
    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div class="block mt-4">
            <label for="remember_me" class="inline-flex items-center">
                {{-- PERBAIKAN: Mengganti warna Indigo pada checkbox ke warna primary (Teal) --}}
        ...
        <div class="flex items-center justify-end mt-4">
            @if (Route::has('password.request'))
                {{-- PERBAIKAN: Mengganti fokus ring ke warna primary --}}
    ...
    {{-- === BAGIAN BARU: LINK REGISTER/DAFTAR === --}}
    @if (Route::has('register'))
    <div class="mt-4 text-center">
        <p class="text-sm text-gray-600">
            Belum punya akun?
            {{-- Menggunakan warna primary yang baru --}}
            <a href="{{ route('register') }}" class="font-semibold text-primary-600 hover:text-primary-700 underline">
                Daftar Sekarang
            </a>
        </p>
    </div>
